...
export function FilterProducts({
  ...
  const filter = async () => {
    ...
    const { totalPages, products } = await filterProduct({
      genders,
      categoriesName,
      sizes,
    });
  ...
  return (
    <>
      <div>
        <div
          className={clsx(
            "w-full flex justify-between outline-none text-sm text-gray-900 border border-gray-300 rounded-lg bg-gray-50 focus:ring-blue-500 focus:border-blue-500",
            {
              "md:mb-6 mb-4 rounded-b-2xl": !isVisible,
              " rounded-b-none": isVisible,
            }
          )}
        >
          <div
          ...
          <div className="flex items-center justify-center mr-2">
            <button
              onClick={filter}
              type="button"
              className="text-white flex flex-row gap-2 bg-blue-700 hover:bg-blue-800 focus:ring-4 focus:outline-none focus:ring-blue-300 font-medium rounded-lg text-sm px-4 py-2"
            >
              Filtrar
            </button>
            {/* <button
              type="button"
              className="bg-blue-600 text-white inline-block md:px-6 px-4 md:m-4 m-1 py-2 md:rounded-md rounded-2xl"
              onClick={filter}
            >
              Filtrar
            </button> */}
                ...
                {categories.map((category, idx) => (
                  <div key={idx}>
                    <input
                      ...
                      onChange={(e) =>
                        handleCheckboxChange(e, "category", category)
                      }
                    />
                    <label
                ...
                {precios.map((precio, idx) => (
                  <div key={idx}>
                    <input
                      ...
                      className="text-blue-600 bg-gray-100 border-gray-300 rounded focus:ring-blue-500"
                      onChange={(e) =>
                        handleCheckboxChange(e, "precio", precio)
                      }
                    />
                    <label
                      htmlFor={`precio-${idx}`}
