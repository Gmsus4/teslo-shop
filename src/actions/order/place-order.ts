    ...
    try {
        const prismaTx = await prisma.$transaction(async(tx) => {
            const updatedProductsPromises = products.map((product) => {
                ...
                const productQuantity = productIds.filter(//Filtra un arreglo donde coincidan los ids
                    p => p.productId === product.id
                ).reduce((currentValue, item) => item.quantity + currentValue, 0)//Suma el valor de la cantidad de esos arreglos 
